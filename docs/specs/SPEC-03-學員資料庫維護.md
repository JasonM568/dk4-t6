...
- 只有 Full Admin 可從人物頁選擇「保留目前卡、合併另一張卡」。兩卡需共享 Email 或手機；手機不同或 claimedUserId 分屬不同會員時拒絕。
- 正式歷史以正規化課名＋日期去重，活動接觸以類型＋標題＋日期去重；非重複子紀錄搬到保留卡，重複子紀錄隨來源卡刪除。
- 來源卡與保留卡都需留下 merge audit；不得移動、合併或刪除 Auth、Enrollment、訂單與場次報名。
- `/admin/people/duplicates` 提供待確認表，只列正規化後同姓名且同 Email 的群組。每張卡顯示手機、認領狀態、課程／活動數、舊站狀態與建立日期。
- 推薦保留順序固定為：已認領會員優先、課程＋活動筆數較多、建立時間較早。推薦只是 UI 建議，每個來源仍需管理員勾選確認後逐筆合併。
...
- `/admin/people/duplicates` 只讓 `buildStudentMergePreview` 判定 `canMerge` 的來源／推薦保留卡組合進入批次勾選；有姓名、Email、手機或會員身分衝突者只保留逐筆檢視，不可勾選。
- Full Admin 每批最多選 20 組，需輸入 `MERGE N`。同一來源不可重複出現，同一張卡不可在同批同時作為來源與保留卡。
- Client 預覽顯示來源、保留卡、預計搬入與重複略過筆數；這只是操作輔助，Server Action 不信任 client 傳入的安全狀態或統計。
- Server 逐組呼叫與單筆合併相同的交易服務，重新載入來源／保留卡並重新計算衝突；每組成功都建立獨立 `StudentMergeOperation`，可在人物頁單獨還原。
- 一組衝突或執行失敗不得回滾、隱藏其他已完成結果。結果報告分為成功、執行時安全略過與失敗，顯示來源名稱、原因及成功操作 ID。
- 批次合併仍不得修改 Auth、Enrollment、訂單、PendingEnrollment 或場次報名，也不得把跨來源會員認領候選當作 StudentRecord 合併。

### T14. 手機衝突人工覆核

- 手機不同的候選一律不視為安全候選、不進批次合併，不依「只差一位數」或其他相似度自動判斷。
- Full Admin 可在逐筆合併表查看兩卡完整手機、會員狀態、課程／活動搬入與略過明細，再填寫至少 4 字覆核原因並完成「已核對兩支手機」及「確認合併」兩次勾選。
- 人工覆核只可放行唯一衝突為 `手機不同`，且正規化姓名相同、Email 相同、兩卡手機皆存在且不同、claimedUserId 未指向不同會員的組合；任何其他衝突仍禁止。
- Server Action 必須重新載入兩卡並重算上述條件，不信任 client 傳入的衝突類型；合併後保留推薦主卡手機，來源手機保存在完整可還原快照。
- `StudentMergeOperation.snapshotJson.manualReview` 與來源／保留卡 audit 都保存覆核類型、原因及當時衝突；人物頁的合併操作區顯示原因。
- 每組仍須逐筆執行及建立獨立還原操作單；本功能不提供「全選手機衝突」或批次人工覆核。
...
| AC-30 | 批次頁只允許無衝突候選勾選，最多 20 組且需輸入正確 `MERGE N` |
| AC-31 | 批次送出後每組重新查核；期間新增衝突者安全略過，不影響其他安全組完成 |
| AC-32 | 每組批次成功各有獨立可還原操作單，結果報告清楚區分成功、略過、失敗與原因 |
| AC-33 | 重複來源、來源／保留鏈式交叉、格式竄改及非 Full Admin 請求在寫入前被拒絕 |
| AC-34 | 手機不同的 13 組候選不進安全批次，無論差異位數多少都必須逐筆人工確認 |
| AC-35 | 唯一衝突為手機不同且姓名／Email／會員身分條件一致時，Full Admin 填原因並二次確認後可合併 |
| AC-36 | 同時存在姓名、Email、claimedUserId 或其他衝突時，即使竄改人工覆核欄位仍被 Server 拒絕 |
| AC-37 | 手機覆核原因與衝突寫入可還原快照及兩端 audit，合併後主卡手機不被來源手機覆蓋 |
