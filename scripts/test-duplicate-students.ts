import assert from "node:assert/strict";import{buildStudentMergePreview,canOverridePhoneConflict,duplicateStudentKey,groupExactDuplicateStudents}from"../src/lib/duplicate-students";
...
const phoneSource={...previewSource(),phone:"0911111111"};const phoneTarget={...previewTarget(),phone:"0922222222"};const phonePreview=buildStudentMergePreview(phoneSource,phoneTarget);
assert.equal(canOverridePhoneConflict(phoneSource,phoneTarget,phonePreview.conflicts),true);
assert.equal(canOverridePhoneConflict({...phoneSource,name:"不同人"},phoneTarget,["姓名不同","手機不同"]),false);
...
function previewSource(){return{id:"source",name:"王小明",email:"a@example.com",phone:null,claimedUserId:null,histories:[],engagements:[]};}
function previewTarget(){return{id:"target",name:"王 小明",email:"A@example.com",phone:null,claimedUserId:"u1",histories:[],engagements:[]};}
