...
export default async function Students({ searchParams }: { searchParams: Promise<{ q?: string; archived?: string }> }) {
  ...
  const [records, archivedCount] = await Promise.all([
    ...
    prisma.studentRecord.count({ where: { archivedAt: { not: null } } }),
  ]);
  ...
  return <div className="max-w-5xl">
    <div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-bold">{showArchived ? "已封存學員名單" : "學員資料庫"}</h1><Link href={showArchived ? "/admin/students" : "/admin/students?archived=1"} className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-50">{showArchived ? "← 回一般名單" : `📦 已封存（${archivedCount}）`}</Link></div>
    ...
    <form className="mb-5 flex gap-2" action="/admin/students">
      ...
      {q && <Link href={showArchived ? "/admin/students?archived=1" : "/admin/students"} className="rounded-lg border border-gray-300 px-4 py-2 text-sm">清除</Link>}
    </form>
    <StudentImport />
    <StudentSync />
    <section className="mt-6 overflow-hidden rounded-xl border border-gray-200">
      <div className="border-b bg-gray-50 px-4 py-3 text-sm text-gray-500">{q ? `查詢「${q}」：${records.length} 位` : showArchived ? `已封存 ${records.length} 位學員` : `最近更新的 ${records.length} 位學員`}</div>
      {records.map((record) => {
