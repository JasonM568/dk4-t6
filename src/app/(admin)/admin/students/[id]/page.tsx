...
export default async function StudentDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="max-w-5xl pb-16">
    ...
    <div className="space-y-5">
      ...
      {isFullAdmin(role) && <><StudentArchiveControl studentId={student.id} archive={student.archivedAt ? { archivedAt: student.archivedAt.toISOString(), archivedBy: student.archivedBy, archiveReason: student.archiveReason } : null} /><section className="rounded-xl border border-red-100 p-5"><h2 className="font-bold">永久刪除</h2><p className="mt-1 text-sm text-gray-500">永久刪除需先檢查會員與影片權限，請前往統一人物頁查看影響範圍並二次確認。</p><Link href={`/admin/people/student/${student.id}`} className="mt-3 inline-block rounded-lg border border-red-300 px-4 py-2 text-sm font-medium text-red-700">前往安全刪除檢查 →</Link></section></>}
