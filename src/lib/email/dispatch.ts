...
async function resolveFollowUpRecipients(record: {
  ...
  // Phase 1 後的新寄送以逐人 ACCEPTED 結果為唯一母集合；舊紀錄沒有逐人列才退回
  // EmailBroadcast.recipients 快照。FAILED 絕不能被誤判成「未開信」。
  const recipientResults = await prisma.emailBroadcastRecipient.findMany({
    where: { broadcastId: record.sourceBroadcastId },
  ...
  let emptyError = "跟進名單目前沒有符合的收件人";
  if (!isFollowUpFilter(record.followUpFilter)) {
    return { error: `跟進條件不正確：${record.followUpFilter}` };
  }
  const emails = resolveFollowUpEmails(record.followUpFilter, acceptedEmails, events);
...
export async function executeBroadcast(broadcastId: string) {
  ...
  // 一次交易以 delete + createMany 批次收斂結果，避免數百位收件人逐筆串行 update。
  // 如果在此之前中斷，先前的 PENDING 會保留下來；不會被誤當 ACCEPTED 或自動重寄。
  if (recipients.length > 0) {
    await prisma.$transaction([
      prisma.emailBroadcastRecipient.deleteMany({ where: { broadcastId } }),
      prisma.emailBroadcastRecipient.createMany({
        data: [
          ...r.acceptedRecipients.map((rcpt) => ({
            broadcastId,
            email: rcpt.email,
            name: rcpt.name,
            status: "ACCEPTED",
            providerMessageId: rcpt.providerMessageId,
          })),
          ...r.failedRecipients.map((rcpt) => ({
            broadcastId,
            email: rcpt.email,
            name: rcpt.name,
            status: "FAILED",
            failureReason: rcpt.reason,
          })),
        ],
      }),
    ]);
  ...
  await prisma.emailBroadcast.update({
    ...
    data: {
      ...
      // 相容快照只保存 provider 已接受者；完整 PENDING/ACCEPTED/FAILED 見逐人結果表。
      recipients: r.acceptedRecipients.map((rcpt) => rcpt.email),
